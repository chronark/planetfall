...
const HTTPS_TEMPLATE_JSON = `{"dns_lookup" : "%s" ,"tcp_connection" : "%s", "ssl_handshake" : "%s", "server_processing" : "%s", "content_transfer" : "%s", "name_lookup" : "%s","connect" : "%s", "pre_transfer": "%s", "start_transfer" : "%s", "total" : "%s"}`;

const HTTP_TEMPLATE =
  "   DNS Lookup   TCP Connection   Server Processing   Content Transfer\n[ %s  |     %s  |        %s  |       %s  ]\n             |                |                   |                  |\n    namelookup:%s      |                   |                  |\n                        connect:%s         |                  |\n                                      starttransfer:%s        |\n                                                                 total:%s\n";

const HTTP_TEMPLATE_JSON = `{"dns_lookup" : "%s" ,"tcp_connection" : "%s", "server_processing" : "%s", "content_transfer" : "%s", "name_lookup" : "%s","connect" : "%s", "start_transfer" : "%s", "total" : "%s"}`;
...
module.exports = (results, options, vercelTimings) => {
  ...
  process.stdout.write(
    sprintf(
      "\n%s%s%s\n",
      url.protocol === "https:" ? colorize("HTTPS", colors.green) : colorize("HTTP", colors.green),
  ...
  Object.keys(res.headers).forEach((key) => {
    console.log(colorize(`${key}:`, colors.white), colorize(res.headers[key], colors.cyan));
  });
  ...
  if (url.protocol === "http:") {
    ...
    console.log(
      sprintf(
        ...
        fmtb(time.onTransfer - time.begin),
        fmtb(vercelTimings.terminator_rt),
        fmtb(vercelTimings.first_byte),
        fmtb(vercelTimings.middleware || 0),
        fmtb(vercelTimings.edgefn || 0),
        fmtb(time.onTotal - time.begin),
      ),
    );
  }
