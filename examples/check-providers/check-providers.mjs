...
async function analyzeHost(url, retry = false) {
  ...
  try {
    ...
    try {
      ...
      console.error(url, pc.red("ERROR"), err.message, err.cause ? err.cause.message : "");
  ...
  if (body.includes('<script src="runtime.') && body.includes('<script src="polyfills.')) {
  ...
  // hosting
  if (res.headers.has("x-vercel-id") || "Vercel" === res.headers.get("server")) {
  ...
  if (res.headers.has("x-nf-request-id") || res.headers.get("server") === "Netlify") {
  ...
  if (res.headers.get("set-cookie")?.includes("msdyn365") || body.includes("_msdyn365")) {
...
if (!SKIP) {
  process.stdout.write(toCSVString(["URL", "Reported Rank", "Rank in CSV", "Tools", "Providers"]));
}
...
for await (const { origin, rank } of stream) {
  ...
  analyzeHost(origin).then(({ frameworks, providers }) => {
    process.stdout.write(
      toCSVString([origin, rank, csvRank, Array.from(frameworks), Array.from(providers)]),
    );
    sema.release();
  });
}
