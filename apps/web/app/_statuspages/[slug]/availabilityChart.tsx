...
export const AvailabilityChart: React.FC<Props> = ({ max, height, days, endpoint, degradedAfter }) => {
...
const Latency: React.FC<{ value: number }> = ({ value }) => {
    return (
        <span className="text-xs text-zinc-700 whitespace-nowrap">{format(value)} ms</span>
    );
};
...
const Expanded: React.FC<{ endpoint: EndpointData }> = ({ endpoint }) => {
    ...
    const table = useReactTable({
        data: Object.values(endpoint.regions).filter(r => r.id !== "global").map((region) => ({
            region: {
                id: region.id,
                name: region.name,
            },
