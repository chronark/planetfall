							...
							<AccordionPrimitive.Header>
								<AccordionPrimitive.Trigger className="flex items-center justify-center w-full">
									...
									<div className="relative flex justify-center">
										<button
											className="px-2 text-zinc-500 hover:text-primary-500"
...
const Chart: React.FC<{
	...
	const errors = timeout ? metrics.filter((m) => m.max >= timeout).length : 0;
	const availability = metrics.length > 0 ? 1 - errors / metrics.length : 1;
	return (
		<div>
			{withAvailability ? (
