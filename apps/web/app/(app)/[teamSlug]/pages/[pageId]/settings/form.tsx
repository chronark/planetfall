...
export const Form: React.FC<Props> = ({ page, teamSlug, endpoints }) => {
  const {
    register,
    formState: { errors },
    handleSubmit,
    ...
    defaultValues: {
      name: page.name,
      slug: page.slug,
      endpointIds: page.endpointIds,
    },
  });

  const [selectedEndpoints, setSelectedEndpoints] = useState<string[]>(page.endpointIds);
    ...
    <form className="space-y-8 divide-y divide-zinc-200">
      ...
      <div className="space-y-8 sm:space-y-5 lg:space-y-24">
        <div className="space-y-6 sm:space-y-5">
          <div>
            ...
            <p className="max-w-2xl mt-1 text-sm text-zinc-500">
              Enter a name and slug for your page. The slug will be used as subdomain:
              slug.planetfall.io
            </p>
          </div>
              ...
              <div className="mt-1 sm:col-span-4 sm:mt-0">
                <div className="">
                  <input
                    ...
                    {...register("slug", {
                      required: true,
                      validate: (value) => new RegExp(/^[a-z0-9-_]+$/).test(value!),
                    })}
                    placeholder="dub"
                    className={`transition-all  focus:bg-zinc-50 md:px-4 md:h-12  w-full ${
                      errors.name ? "border-red-500" : "border-zinc-700"
        ...
        <div className="pt-8 space-y-6 divide-y divide-zinc-200 sm:space-y-5 sm:pt-10">
          <div>
            <h3 className="text-lg font-medium leading-6 text-zinc-900">Endpoints</h3>
            <p className="max-w-2xl mt-1 text-sm text-zinc-500">
              ...
              <div role="group" aria-labelledby="label-email">
                <div className="sm:grid sm:items-baseline sm:gap-4">
                  <div className="mt-4 sm:col-span-3 sm:mt-0">
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (selectedEndpoints.includes(e.id)) {
                              setSelectedEndpoints(selectedEndpoints.filter((id) => id !== e.id));
                            } else {
                              setSelectedEndpoints([...selectedEndpoints, e.id]);
                            }
                          }}
                        >
                          <span className="font-medium text-zinc-700">{e.name}</span>
                    ...
                    {errors.endpointIds ? (
                      <p className="mt-2 text-sm text-red-500">
                        {errors.endpointIds.message || "Select at least one endpoint"}
                      </p>
                    ) : null}
                  </div>
                </div>
