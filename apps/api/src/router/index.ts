...
router.route({
      ...
      200: z.array(
        z.object({
          id: z
            .string()
            .describe(
              "The endpoint's id. You can see this in the URL when you open the endpoint in the dashboard.",
            ),
  ...
  handler: async (req, ctx) => {
    ...
    const endpoints = await kysely(ctx.env.DATABASE_URL)
      .selectFrom("Endpoint")
      .selectAll()
...
router.route({
  ...
  handler: async (req, ctx) => {
    const { policy, team } = await ctx.authorize(req.headers.get("authorization"));
    const access = policy.validate(
      "endpoint:read",
      `${team.id}::endpoint::${req.params.endpointId}`,
    );
    if (!access.valid) {
    ...
    responses: {
      ...
      200: z.object({
        ...
        slug: z
          .string()
          .describe(
            "The team's slug. This is used in the url when you open the team in the dashboard.",
          ),
        createdAt: z
          .number()
          .describe(
            "The timestamp when the team was created. This is a unix timestamp in milliseconds.",
          ),
        plan: z.enum(["FREE", "PRO", "ENTERPRISE"]).describe("The team's current plan."),
        maxMonthlyRequests: z
          .number()
          .describe("The maximum number of requests the team can make per month."),
        maxEndpoints: z.number().describe("The maximum number of endpoints the team can create."),
        maxTimeout: z
          .number()
          .describe("The maximum timeout in milliseconds the team can set for an endpoint."),
        maxPages: z
          .number()
          .describe("The maximum number of pages the team can set for an endpoint."),
      }),
    },
  } as const,
  handler: async (req, ctx) => {
    const {
      team: { id },
    } = await ctx.authorize(req.headers.get("Authorization"));
