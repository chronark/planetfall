...
export const getParamsFromPath = (pattern: string, input: string): Record<string, any> => {
 ...
 */
export type PathParamNames<Path, Acc = never> = Path extends `${string}:${infer Name}/${infer R}`
