...
export const getEndpoints = makeRequestHandler({
  ...
  run: async ({ request, input, sendOutput }) => {
    ...
    const endpoints = await kysely
      .selectFrom("Endpoint")
      .selectAll()
      .where("Endpoint.teamId", "=", auth.teamId)
      .execute();
    ...
    const endpointsWithAccess = endpoints.filter((endpoint) => {
      ...
      if (!access.valid) {
        console.warn(
          `Team ${auth.teamId} tried to access endpoint ${endpoint.id} but was denied access: ${access.error}`,
        );
        return false;
      }
      return true;
    ...
    const endpointsWithRegions = await Promise.all(
      endpointsWithAccess.map(async (endpoint) => {
        const regions = await kysely
          ...
          .where("_EndpointToRegion.A", "=", endpoint.id)
          .execute();
