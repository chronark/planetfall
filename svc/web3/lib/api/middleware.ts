...
export function withMiddleware(handler: NextApiHandler, ...middlewares: Middleware[]): NextApiHandler {
    ...
    return (h: NextApiHandler) =>
        ...
        (req, res) => {
            if (!req.method || !methods.map(m => m.toUpperCase()).includes(req.method?.toUpperCase())) {
                throw new ApiError({
                    status: 405, message: `Method ${req.method} not allowed`
                })
            }
