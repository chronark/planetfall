...
export default async function handler(
    ...
    try {
        ...
        const org = await clerk.organizations.createOrganization({
            name: "Personal",
            slug,
            createdBy: request.data.body.data.id,
        })
        ...
        const customer = await stripe.customers.create({
            email: request.data.body.data.email_addresses && request.data.body.data.email_addresses.length > 0 ? request.data.body.data.email_addresses[0].email_address : undefined
        })
