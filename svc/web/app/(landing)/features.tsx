...
const features: Feature[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      { title: "CI-CD", description: "Use preview branches and add monitoring within your CI pipeline" },
    ],
  },
];
...
const Feature: React.FC<{ feature: Feature }> = ({ feature }) => {
  return (
    <div className="min-h-screen mx-auto text-center container lg:px-8">
      ...
      {feature.image ?
        <div
          className="mx-auto  mt-4 md:mt-8 lg:mt-16 border rounded overflow-hidden border-primary-600 drop-shadow-feature"

        >
          <Image src={feature.image} width={1920} height={1080} alt="Feature screenshot" />
        </div>
        : null}
  ...
  return (
    <section id="features">
      <div className="relative py-16 sm:py-24 lg:py-32 space-y-8 md:space-y-16 lg:space-y-32">
        {features.map((f) => <Feature key={f.title} feature={f} />)}
      </div>
    </section>
  );
}
